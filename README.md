...
备注：
	1.MySQL目录下脚本自动获取CDB信息（项目名，实例名，ip地址，备份状态，磁盘使用率等）汇总并发送邮件，方便查看与管理  
	
	2.Redis目录下脚本自动获取redis信息（项目名，实例名，ip地址，内存使用率等）汇总并发送邮件，方便查看与管理  
	
	3.Disaster Recover 目录下脚本进行容灾演练切换  
